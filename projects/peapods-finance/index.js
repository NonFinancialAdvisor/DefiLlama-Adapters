...
const getTvl = async (api, isStaking) => {
  ...
  const ownerTokens = [];
  const indexes = (
    await api.call({ abi: indexManagerABI, target: indexManager })
  ).map((i) => i.index);

  //get staking pool contract for index (spp)
  const stakingPools = await api.multiCall({
  ...
  stakingTokens.forEach((stakingToken, i) =>
    ownerTokens.push([[stakingToken], stakingPools[i]]),
  );
  await sumTokens2({
    api,
    ownerTokens,
